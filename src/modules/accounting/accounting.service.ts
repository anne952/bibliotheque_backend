...
export class AccountingService {
   ...
   */
  static async getTrialBalance(fiscalYearId: string): Promise<TrialBalance[]> {
    ...
    const balances: Map<string, TrialBalance> = new Map();
   ...
   */
  static async getIncomeStatement(fiscalYearId: string) {
    const balances = await this.getAccountBalances(fiscalYearId);

    const revenues = balances.filter(
      (b) => parseInt(b.accountNumber) >= 700,
    );
    const expenses = balances.filter(
      (b) =>
        parseInt(b.accountNumber) >= 600 &&
   ...
   */
  static async getPreviousPeriodBalance(
    accountId: string,
    beforeFiscalYearId: string,
  ): Promise<number> {
    const lines = await prisma.journalLine.findMany({
      where: {
        accountId,
