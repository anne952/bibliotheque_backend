...
class ReportsService {
    static resolveDashboardRange(from, to) {
        const toDate = to ? new Date(to) : new Date();
        if (Number.isNaN(toDate.getTime())) {
            throw new http_1.AppError("Paramètre 'to' invalide", 400);
        }
        const fromDate = from ? new Date(from) : new Date(toDate);
        if (Number.isNaN(fromDate.getTime())) {
            throw new http_1.AppError("Paramètre 'from' invalide", 400);
        }
        if (!from) {
        ...
        if (fromDate > toDate) {
            throw new http_1.AppError("La période est invalide: from doit être <= to", 400);
        }
        return { fromDate, toDate };
    }
    static async getDashboardOverview(options) {
        const { fromDate, toDate } = this.resolveDashboardRange(options?.from, options?.to);
        const [salesCount, purchasesCount, donationsCount, loansCount, returnsCount, salesSum, purchasesSum, donationFinancialInSum, materials, totalPersons, donorsCount, suppliersCount, buyersCount, borrowersCount, validatedEntriesCount, unvalidatedEntriesCount, mostBorrowedRaw, donorFinancialRaw,] = await Promise.all([
            ...
            prisma_1.prisma.saleItem.aggregate({
                where: {
                    sale: {
                        saleDate: { gte: fromDate, lte: toDate },
                    },
            ...
            prisma_1.prisma.purchaseItem.aggregate({
                where: {
                    purchase: {
                        purchaseDate: { gte: fromDate, lte: toDate },
                    },
            ...
            prisma_1.prisma.person.count({ where: { deletedAt: null } }),
            prisma_1.prisma.person.count({ where: { deletedAt: null, isDonor: true } }),
            prisma_1.prisma.person.count({ where: { deletedAt: null, isSupplier: true } }),
            prisma_1.prisma.person.count({ where: { deletedAt: null, isBuyer: true } }),
            prisma_1.prisma.person.count({ where: { deletedAt: null, isBorrower: true } }),
            prisma_1.prisma.journalEntry.count({
                where: {
            ...
            prisma_1.prisma.loanItem.groupBy({
                by: ["materialId"],
                _sum: { quantity: true },
                _count: { _all: true },
                orderBy: {
        ...
        const lowStockCount = materials.filter((m) => m.currentStock > 0 && m.currentStock <= m.minStockAlert).length;
        ...
        const borrowedMaterials = borrowedMaterialIds.length
            ...
            : [];
        const borrowedMaterialById = new Map(borrowedMaterials.map((material) => [material.id, material]));
        const donorIds = donorFinancialRaw
            .map((entry) => entry.donorId)
            .filter((id) => Boolean(id));
        const donors = donorIds.length
            ? await prisma_1.prisma.person.findMany({
    ...
    static async getDashboardActivity(limit = 20) {
    ...
    static async getDashboardStockAlerts(limit = 10) {
